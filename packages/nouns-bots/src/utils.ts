...
export function formatAuctionStartedTweetText(auctionId: number) {
 ...
 Learn more at https://unouns.wtf`;
}
...
export async function formatBidMessageText(id: number, bid: Bid) {
  ...
  return `UNoun ${id} has received a bid of Ξ${ethers.utils.formatEther(bid.amount)} from ${bidder}`;
}
...
export function getAuctionEndingSoonTweetText() {
  return `This auction is ending soon! Bid now at https://unouns.wtf`;
}
